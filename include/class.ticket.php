...
/* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
    ...
    function getCCEmails(){
        return $this->ht['cc_emails'];
    }
    
    function getTicketEmailsArray(){
        $ticket_emails_array = array($this->ht['email']);
        if($this->getCCEmails() != '')
        {
            $cc_emails = explode(',', $this->getCCEmails());
            $ticket_emails_array = array_merge($ticket_emails_array, $cc_emails);
        }
            
        //var_dump($ticket_emails_array);
        return $ticket_emails_array;
    }
    /* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
...
/*Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD
needed to enable staff to modify cc emails */
...
/* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
    ...
    function onNewTicket($message, $autorespond=true, $alertstaff=true) {
        ...
        if($autorespond && $email && $cfg->autoRespONNewTicket()
            ...
            $email->sendAutoReply($this->getEmail(), $msg['subj'],
                $msg['body'], null, $options, $this->getCCEmails());
            /* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
        }
    ...
    function onMessage($autorespond=true, $message=null, $to_list, $cc_list) {
        global $cfg;
       ...
       /**********   double check auto-response  ************/
        ...
        /* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
            ...
            $options = array('references' => $message->getEmailMessageId());
/* Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
              $email->sendAutoReply($to_list, $msg['subj'], $msg['body'],
                null, $options, $cc_list);
            /* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
        }
    }
...
/*Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD
we do not use auth token any more - do not send it in email messages sent to users, only ticket ID is needed */
...
/* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
	...
	/* Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
	if($vars['email']){  //message sent by email
          /*note: we want to send autoreply only to who has not sent or already received the message via email
          note: $vars['email'] here does not necessarily correspond to ticket's owner email, 
          and ticket's owner may have been CCed here (message may have been sent by a CC)
          so we merge all addresses to get all the people who already sent/received the message via email */	
          $all_emails=$vars['cc_emails']?$vars['email'].','.$vars['cc_emails']:$vars['email'];
          self::update_CC_email_addresses_in_db($this->getEmail(), $all_emails); 
          $this->reload;
          $all_emails_array=self::explode_email_list($all_emails);
          $cc_emails_array=self::explode_email_list($this->getCCEmails());
          foreach($cc_emails_array as $cc_email){
           if(!in_array($cc_email,$all_emails_array))
            $cc_list.=$cc_list?','.$cc_email:$cc_email;
          }
          if(!in_array($this->getEmail(),$all_emails_array)){
             $to_list=$this->getEmail();
          }else{ 
           $to_list=$cc_list;  // we cannot leave the "to:" field empty, so we use CCs 
           $cc_list='';
          }         
        }else{
    ...
    function postReply($vars, &$errors, $alert = true) {
        ...
        if($tpl && ($msg=$tpl->getReplyMsgTemplate()) && $email) {
            ...
            /* Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD (taken from WALTEREGO CC MULTIPLE EMAILS) */
            $email->send($this->getEmail(), $msg['subj'], $msg['body'], $attachments, $options, $this->getCCEmails());
            /* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
        }
...
/* Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD
when a ticket is deleted, delete also cc emails associated with it */
        $sql = 'DELETE FROM '.TICKET_CC_EMAILS_TABLE.' WHERE ticket_id='.$this->getId();
        db_query($sql); // TODO insert some check here?
/* End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD */
...
// Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD
        $fields['cc_emails']= array('type'=>'email_list','required'=>0,'error'=>/*__(*/'invalid cc email address(es) provided'/*)*/);
// End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD
        ...
        self::create_password($vars['email']);
        self::update_CC_email_addresses_in_db($vars['email'], $vars['cc_emails'], true);
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    function getClientStats($email) {
        ...
        /* Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD (taken from WALTEREGO CC MULTIPLE EMAILS) */
        $sql='SELECT count(DISTINCT(open.ticket_id)) as open, count(DISTINCT(closed.ticket_id)) as closed '
            .' FROM '.TICKET_TABLE.' ticket '
            .' LEFT JOIN '.TICKET_TABLE.' open
...
// Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD
        $fields['cc_emails']= array('type'=>'email_list','required'=>0,'error'=>/*__(*/'Invalid cc email address(es)'/*)*/);
// End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD
...
// Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD            
        self::create_password($vars['email']);
        self::update_CC_email_addresses_in_db($vars['email'], $vars['cc_emails'], false, $id);
// End EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD        
...
// Start EDIT for CC_EMAILS+BASIC_CLIENT_AUTH MOD   
   function create_password($email){
        global $cfg;   
        // If client's email has not a password yet, create it and send to client's email (TODO: it should be better to send an activation link, and let user choose her/his own password)
        $sql='SELECT * FROM '.CLIENT_PASSWORDS_TABLE.' WHERE email='.db_input($email);
        if(!($res=db_query($sql)) || !db_num_rows($res)) {  
        	$rpwd = self::random_password();
        	...
        	if(db_query($sql)){ 
        		...
        		$message=str_replace('nwln', "\n", sprintf(__('A new account has been automatically created for %1$s on %2$s.nwlnnwlnYour password is %3$snwlnnwlnYou can login at %2$slogin.phpnwlnnwlnWe suggest that you change your password at first login by clicking on Change Password'),$email,$cfg->getUrl(),$rpwd));
        		Mailer::sendmail($email, $subject, $message, $cfg->getAlertEmail()->getEmail());
        	}
        	unset($rpwd);
        	// TODO: some logging and errors handle needed here
        }
   }
   ...
   function update_CC_email_addresses_in_db($ticket_email, $cc_emails, $delete=false, $ticket_id=null){
   
        //IF THERE ARE OTHER EMAILS, ADD THEM TO THE CC_EMAILS TABLE AND DELETE THOSE THAT ARE NOT INCLUDED ANY MORE IN THE LIST
        
        $cc_emails_array=self::explode_email_list($cc_emails);      
        if($ticket_id){ // new ticket
         $cc_emails_db_array=array();
        }else{ //updating existing ticket
         $cc_emails_db_array=self::get_cc_emails_from_db();
        ...
        {          
                 ...
                 && (!in_array($cc_email, $cc_emails_db_array)))
            {
                 $sql='INSERT INTO '.TICKET_CC_EMAILS_TABLE
                     ...
                     .' ,email='.db_input($cc_email);
      
                if(db_query($sql))
                	self::create_password($cc_email);
            }
        }
         
        if($delete){
           foreach($cc_emails_db_array as $cc_db_email){  // delete cc_emails in db which are not included any more in the list
             if(!in_array($cc_db_email, $cc_emails_array)){
                $sql='DELETE FROM '.TICKET_CC_EMAILS_TABLE
                	   .' WHERE ticket_id='.db_input($this->getId())
                	   .' AND email='.db_input($cc_db_email);
                db_query($sql);
             }
           }
        }
   ...
   function get_cc_emails_from_db(){
        $sql='SELECT GROUP_CONCAT(email) AS cc_emails FROM '.TICKET_CC_EMAILS_TABLE
        .' WHERE ticket_id='.db_input($this->getId());
        $res=db_fetch_array(db_query($sql));
        $res['cc_emails'] = strtolower($res['cc_emails']);
        $cc_emails_db_array = explode(',', ($res['cc_emails']));
        return $cc_emails_db_array;
   
   }
   
   function explode_email_list($email_list){
